#include "src/pc_emulator/include/insns/xor_insn.h"
 ...
 */
void XOR_Insn::Execute(std::vector<PCVariable*>& Operands, bool isNegated) {
    auto Logger = __AssociatedResource->__configuration->PCLogger.get();

    if (Operands.size() != 1) {
        Logger->RaiseException("XOR can take exactly one argument ! "
    ...
    if (!Utils::IsBitType(CurrentResult->__VariableDataType) ||
        ...
        Logger->RaiseException("XOR: Only Bit datatypes are supported");
    }
