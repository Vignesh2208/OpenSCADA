#include "src/pc_emulator/include/insns/eq_insn.h"
 ...
 */
void EQ_Insn::Execute(std::vector<PCVariable*>& Operands, bool isNegated) {
    auto Logger = __AssociatedResource->__configuration->PCLogger.get();

    if (Operands.size() != 1) {
        Logger->RaiseException("EQ can take exactly one argument ! "
    ...
    if (Operand->__VariableDataType != CurrentResult->__VariableDataType) {
        ...
        if(DesiredDataType == nullptr) {
            Logger->RaiseException("LT: Typecasting error!");
        }
