#include "src/pc_emulator/include/sfc/sqrt.h"
...
void SQRT::Execute(std::vector<PCVariable*>& Operands) {
    ...
    if (!Utils::IsRealType(CR->__VariableDataType)) {
        configuration->PCLogger->RaiseException("SQRT SFC error: CR is not "
            " a real number");
    }
    ...
    if (CR->__VariableDataType->__DataTypeCategory == DataTypeCategory::REAL) {
        ...
        if (RealValue < 0)
            configuration->PCLogger->RaiseException("SQRT exception: negative domain");
        RealValue = sqrt(RealValue);
