#include "src/pc_emulator/include/sfc/asin.h"
...
void ASIN::Execute(std::vector<PCVariable*>& Operands) {
    ...
    if (!Utils::IsRealType(CR->__VariableDataType)) {
        configuration->PCLogger->RaiseException("ASIN SFC error: CR is not "
            " a real number");
    }
